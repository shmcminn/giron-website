...
cmsmastersShortcodes.cmsmasters_timetable = { 
	...
	fields : { 
		...
		booking_popup_message : { 
			...
			def : 		"<h2>You are about to book event</h2><p>{event_title}</p><p>{column_title}</p><p>{event_start} - {event_end}</p><p>An initial receipt will be sent out automatically unless you decide not to do so below.</p><div>{tt_btn_book}{tt_btn_cancel}</div>", 
			required : 	false, 
			width : 	'full' 
		}, 
		...
		booking_popup_thank_you_message : { 
			...
			descr : 	cmsmasters_timetable_shortcodes.timetable_field_booking_popup_message_descr + ' {event_title} {column_title} {event_start} {event_end} {event_description_1} {event_description_2} {user_name} {user_email} {tt_btn_book} {tt_btn_cancel} {tt_btn_continue}', 
			def : 		"<h2>Thank you for choosing our services!</h2><p>{event_title}</p><p>{column_title}</p><p>{event_start} - {event_end}</p><p>This is a confirmation of your booking. Your booking is now complete and a confirmation email has been sent to you.</p><div>{tt_btn_continue}</div>", 
			required : 	false, 
			width : 	'full' 
		}, 
